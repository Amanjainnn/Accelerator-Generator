...
@Slf4j
public class SpringBootModuleGenerator implements ModuleGenerator {

	@Override
	public void generate(Path path, TechStackRequest request)
			throws BadRequestException, IOException, InterruptedException {
		String commandToGenerateServer = " generate" + " -g spring " + " -i "
				+ Path.of(new ClassPathResource("default.yml").getURI()) + " -o "
				+ path.resolve(request.getAppName() + "Backend") + " -p groupId=" + request.getForm().get("groupId")
				+ " -p artifactId=" + request.getForm().get("artifactId") + " -p artifactVersion=1.0.0-SNAPSHOT "
				+ " -p basePackage=" + request.getForm().get("groupId");
